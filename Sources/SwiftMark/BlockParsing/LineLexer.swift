    ...
    fileprivate static func readListMarker(_ scanner: inout Scanner<View>) throws -> ListKind {
        ...
        try scanner.popWhile { token in
            guard let token = token, token != Codec.linefeed else {
                throw ListParsingError.notAListMarker // e.g. - followed by end of line / end of string
            }
...
struct LineLexerContext<C: MarkdownParserCodec> {
    ...
    var documentStart: Bool
    
    static var `default`: LineLexerContext {
        return .init(listKindBeingRead: nil, documentStart: true)
    }
}
        ...
        while let token = scanner.pop(ifNot: Codec.linefeed) {
            ...
            case .endSpaces:
                if token == Codec.space {
                    ...
                    state = .hashes
                    end = scanner.data.index(before: scanner.startIndex)
                } else {
                    state = .text
                    end = scanner.startIndex
                }
...
extension MarkdownParser {
    static func parseLine(_ scanner: inout Scanner<View>, context: LineLexerContext<Codec>) -> Line {
        ...
        case Codec.plus, Codec.zero...Codec.nine:
            // FIXME: Teacher's Notes
            // Single + -> List
            // Double ++ -> Teacher's Notes
            // > 2 -> Paragraph
            return parseList(&scanner, indent: indent, context: .default)
